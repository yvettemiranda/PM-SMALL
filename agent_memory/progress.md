...
- 用户已明确授权开始累计 72 小时正式 TEST；按既定每 4 小时与配置变化分段、逐段由用户决定计入的方案执行。
- 首次 `start` 在调用 TEST 启动前因监控证据目录为 `root:root / 0700` 而安全失败；策略与账本保持 PAUSED/健康，失败监控已停止且现场保留。当前正在修复镜像运行用户的目录权限交接并重新验证。
