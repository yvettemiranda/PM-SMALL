...
- 用户已明确授权开始累计 72 小时正式 TEST；正式活动 `formal-test-20260808T081226Z` 已运行，按既定每 4 小时与配置变化分段、逐段由用户决定计入的方案执行。
...
- 已创建并启动正式 TEST 活动 `formal-test-20260808T081226Z`：启动前基线 `/home/ubuntu/pm-small-formal-test-baseline-20260808T081222Z` 的 SHA-256 全部通过，证据目录所有权与镜像运行用户一致；监控容器 `running`、重启 0、日志无错误。
- 启动后复核为 `TEST / LIVE=False / RUNNING`，账本验证 `passed=true`、SQLite `ok`；行情连接正常并订阅 6,236 个 Token，扫描链路无限流、瞬态错误或行情断线，随后产生 13 个 TEST 持仓，证明发现、盘口、仲裁与模拟执行链路已实际运行。
- 当前 `segment-0001` 从 2026-08-08 16:12:27 开始，首个节点为 20:12:27；当前累计仍为 `0.00 / 72.00` 小时，任何已完成片段均须等待用户明确计入或排除。
...
- 正式 TEST 正在服务器自主运行，活动为 `formal-test-20260808T081226Z`、当前片段为 `segment-0001`；关闭 Codex 不会停止服务器侧 TEST 或监控。
- 最近独立复核为 `TEST + LIVE_DISABLED + RUNNING`、账本验证通过、SQLite `ok`、监控重启 0；首个完成节点前没有待用户决定的片段，累计计入仍为 0 小时。
- 用户回来询问时先执行正式状态读取，再报告安全状态、当前/已完成片段、采样健康、扫描/行情、模拟成交与资源异常；只有用户明确确认后才能执行 include 或 exclude。

## 后续步骤

1. 在首个 4 小时节点之后，用户下次询问时读取 `formal-test-campaign.sh status` 和原始证据，报告 `segment-0001` 健康摘要并询问是否计入 72 小时。
2. 后续每个 4 小时或配置变化片段均先向用户报告，由用户明确决定计入或排除；钱包和 LIVE 仍不在当前授权范围内。
