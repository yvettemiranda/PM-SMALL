...
- 旧活动已记录 11.33 小时：`segment-0001` 与 `segment-0002` 各 4.00 小时已计入，`segment-0003` 为 3.33 小时 `REJECTED`；旧证据未被改写。跨活动正式累计为 `8.00 / 72.00` 小时。
- 新活动于 2026-08-09 16:38:06（Asia/Shanghai）启动，首段 `segment-0001` 的固定 4 小时节点约为 20:38；活动内部当前计入 0 小时，只有用户后续确认的合格片段才与旧活动 8 小时合并。
- 服务器最终复核为 `TEST + LIVE_DISABLED + RUNNING`、SQLite `ok`、即时验证通过；业务容器健康、监控重启 0，用户可以关闭 Codex，服务器继续采样。
...
1. 用户以后询问进度时读取新活动状态，并把“旧活动已计入 8 小时”与“新活动已确认时长”合并报告。
2. 每个新片段仍按原规则等待用户决定；新活动累计确认 64 小时后，总计达到 72 小时，应立即受控停止并进入最终审计。
