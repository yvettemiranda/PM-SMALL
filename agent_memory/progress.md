...
- 用户已授权暂停、最小修复目标卖单碎片、保留已计入的 28.00 小时并在验证后同步 GitHub/服务器；正式活动已停止且策略确认为 `TEST + LIVE_DISABLED + PAUSED`，第 6 段约 1.33 小时维持 `PENDING`、未计入。
...
- 新活动于 2026-08-09 16:38:06（Asia/Shanghai）启动；现已按用户授权安全停止，监控活动为 `STOPPED`，服务器为 `TEST + LIVE_DISABLED + PAUSED`。`segment-0006` 约 1.33 小时保持 `PENDING` 且不计入，跨活动确认累计仍为 `28.00 / 72.00` 小时。
...
- 审计发现的目标单碎片漏卖已进入最小修复验证：同 Token 的有序 target 可聚合为一笔满足平台最小量的 FAK SELL，批次限价取最高 target 以避免越过任一目标价，成交与费用再按稳定顺序分配回原 target；修复尚未发布或恢复正式 TEST。
...
1. 提交并核对 GitHub `main`，随后先备份服务器 SQLite 与配置，再部署同一 SHA；在 PAUSED 状态完成账本、SQLite、容器、公网及 Canada-like 退出验证。
2. 恢复正式 TEST 的新活动/新片段，明确继承跨活动已确认的 `28.00 / 72.00` 小时；`segment-0006` 未经用户决定继续保持不计入。
