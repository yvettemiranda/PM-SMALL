...
- 排序修改、回归验证、GitHub 与服务器同步均已完成；当前等待用户设置筛选项，只有用户再次明确通知才开始长期 TEST。
...
- 扫描 Event 已按用户确认的两级顺序发布：可交易整体在前，待定整体在后，进度按钮只控制各组内部；GitHub、Linux 运行提交和实际 197 项扫描顺序均已核对。
- 服务器最终复核为 `TEST + LIVE_DISABLED + PAUSED`、100U 初始/可用资金、空订单/持仓/待结算、SQLite `ok`；监控容器为空且不存在 `formal-test-current`，确认没有启动正式 TEST。
- 服务器现保留旧验证备份 `/home/ubuntu/pm-small-backup-20260808T012255Z` 与本轮更新备份 `/home/ubuntu/pm-small-backup-20260808T040744Z`；不得未经另行盘点和授权删除。
- 当前等待用户完成筛选配置；用户明确发出开始指令前，不启动累计 72 小时 TEST，不创建活动或监控容器。
