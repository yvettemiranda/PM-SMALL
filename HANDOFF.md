...
PM-SMALL 已完成本轮最终 TEST UI、扫描稳定性和大集合性能优化的本地实现、自动验证、真实多轮扫描及手机浏览器检查；当前正在发布前双轴审查阶段，尚未把本轮提交推送 GitHub 或部署 Linux。服务器仍运行上一轮 GitHub 功能提交 `40a09e36a9cb1bd27f3b39a11bc96b71083ba4ee`；所有环境只允许 `TEST + LIVE_DISABLED`：不接钱包、不签名、不发送真实订单、不执行链上写操作。
...
2026-08-07 又在已核对的 `origin/main` `b9143b921fec3e366034faa29be04d84529093c1` 上完成本地 UI 与扫描优化：Polymarket 首页栏目动态同步并默认全选，总时长改为自由上下限，资金摘要/排序/持仓折叠按浏览器注释精简；开放事件改为流式处理，静态条件先筛选，再有界并发拉盘口；普通重扫改为同连接增量订阅，断线候选保留上次报价但禁止交易。隔离预览已升级到 Schema 14 并保持 `TEST + LIVE_DISABLED + PAUSED`；这些改动尚未推送 GitHub、尚未部署服务器，不构成正式远端进度。
...
- 发布前 Standards / Spec 双轴代码审查正在进行；上一轮 UI 审查发现的真实 Schema 13→14 迁移、栏目挂起不阻塞与 5 秒回退、旧 Schema 13 soak 兼容和窄屏长金额防护已保留。
...
- 本轮 Standards / Spec 双轴审查、GitHub `main` 推送和 Linux 服务器同 SHA 部署。
...
1. 完成发布前双轴审查并修复全部发现，推送 GitHub `main`。
2. 停止服务器容器，备份并校验数据和配置，部署同一 SHA；只做短时接线/安全冒烟并保持 `PAUSED`。
3. 完成交付后停止继续运行验证，由用户制定正式长期 TEST 验证计划；任何失败都保留现场并维持 LIVE 禁用。
