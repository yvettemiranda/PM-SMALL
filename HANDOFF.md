...
本轮有效代码包括：可配置 `0.1¢–99¢` 买价闭区间、可配置目标卖价参数 `A/B`、紧凑单列 UI、五项资金摘要、默认折叠的真实交易记录，以及相应的 Schema 16 迁移、API、执行接线和回归。交易记录按真实成交批次而不是毫秒时间戳归组，因此同一卖单在同一毫秒内连续部分成交和最终成交不会被误合并；同一批次跨多个价位的 Fill 仍合并为一条。宽价格全量 Event 校正按有界时间片让出事件循环，并只缓存展示/重算需要的紧凑摘要，不长期保留整本盘口、Preview 或 Intent。

本轮本地验证为 32 个测试文件、310 项测试全部通过；其中包含 1,500 个宽价格 Event 的事件循环响应、暂停生效和紧凑缓存容量回归。TypeScript 类型检查、生产构建、前端语法、正式 TEST 包装脚本语法和差异检查通过。GitHub 与服务器的最终状态不得仅凭本文件推断，交接时必须实时核对 `origin/main`、服务器 `git rev-parse HEAD`、Schema、状态端点和正式活动证据。
