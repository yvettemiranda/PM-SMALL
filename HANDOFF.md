...
2026-08-07 又在已核对的 `origin/main` `b9143b921fec3e366034faa29be04d84529093c1` 上完成一轮本地 UI 调整：Polymarket 首页栏目动态同步并默认全选，总时长改为自由上下限，资金摘要/排序/持仓折叠按浏览器注释精简。隔离预览已升级到 Schema 14 并保持 `TEST + LIVE_DISABLED + PAUSED`；这些改动尚未推送 GitHub、尚未部署服务器，不构成正式远端进度。
...
6. 类别只显示 Polymarket 首页顶栏当前栏目并保存官方 Tag ID；程序按公开栏目接口动态同步，默认全选且自动纳入新栏目，一个市场可以属于多个 Tag。没有官方标签的市场在“全选”模式下仍可参与，但不会生成伪造的 `OTHER` 类别。
...
- GitHub `main` 和服务器当前仍为 Schema 13。这个本地未推送分支为 Schema 14：在 Schema 13 基础上新增 `min_market_duration_days`，并解除旧版最大时长离散节点限制，使最短/最长值都可在 1–365 天内保存。
...
- 本轮 Standards / Spec 双轴代码审查已完成；补齐真实 Schema 13→14 迁移、栏目挂起不阻塞与 5 秒回退、旧 Schema 13 soak 兼容和窄屏长金额防护后，最终复审均为无 findings。
- 本轮本地 UI 已在 462px Codex 侧边栏检查：14 个首页栏目默认全选、7–30 天自由输入、三项资金摘要、单排序按钮和无横向溢出均通过，控制台无错误。
