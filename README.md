...
- 具体市场总时长固定至少 1 天，上限由滑杆选择；默认 30 天即 `1 天 <= 总时长 <= 30 天`。生命周期进度是可配置硬门槛，默认 `<=20%`。
- 支持二元/三元和官方 Tag ID 类别多选；没有官方标签的市场仅在“全部类别”下参与，不制造伪类别，也没有历史成交量硬过滤。
...
- 配置包括官方类别、二元/三元、1–3¢ 买价、Bid/Ask 比例、最大生命周期进度、总时长、总模拟资金和每轮金额；
