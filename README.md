...
- 未锁定 Event 只有在全部静态合格兄弟 Token 都取得新鲜完整 Book 后才仲裁；完整 Book 但无有效 Bid/Ask 只淘汰该 Token。仲裁使用与真实执行共用的无副作用 FAK Preview，并按退出就绪度、目标 Bid 深度覆盖、资金可成交比例、Bid/Ask、费后目标收益率、生命周期及稳定身份字段确定唯一 Winner。
...
- 当前持仓显示 Event、active Token、冻结预算、已用金额和累计/退出状态；持仓与扫描 Event 均默认显示前 20 项并可展开，扫描排序使用一个小按钮切换生命周期进度正序/倒序；
