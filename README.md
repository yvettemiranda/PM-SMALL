...
- 事件分页和订单簿批次对限流、传输及临时服务错误执行有界只读重试，不从头丢弃已完成的扫描进度；
- 2至3个明确结果的事件过滤；
...
- 市场生命周期进度超过用户保存的上限后撤销买单未成交部分并停止新买；进度上限支持`1%`至`100%`，默认20%；
...
- 实时记录全市场扫描阶段、分页/批次数、请求/重试/限流次数、规模和耗时，并提供JSONL长期PAPER留证运行器；
...
- 提供纸面持仓、结算和赎回状态接口；
- 完成手机优先的单列TEST界面：展示资金盈亏、全部当前持仓和扫描市场，支持二元/三元、`1¢`至`3¢`最高买价、至少1天的离散总时长、`1%`至`100%`生命周期进度上限、进度排序及Token结果方向选择；
- 扫描结果逐市场默认全选并可多选；待成交自动买入委托与已成交持仓分开展示，异步按钮和扫描过程显示转圈、经过秒数及最近完成时间；
- 提供`开始TEST / 暂停TEST`和显式`开始新一轮`：普通开始只续跑当前账本，新一轮只解锁已完整卖出且未正式结算的Token，持仓、卖单、资金及历史盈亏继续保留；
- 配置保存后立即约束自动及手动PAPER新买单，并在同一SQLite事务中撤销不再符合新筛选的活动买单；TEST选择持久化，LIVE入口继续锁定；
- 固定禁用的实盘执行器。
