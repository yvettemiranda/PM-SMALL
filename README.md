...
- 先按结果数、首页栏目、自由时长区间、生命周期进度和体育开赛等静态条件预筛，再以有界并发拉取盘口；全部静态合格 Token 进入增量 WebSocket 监控，包括暂时价格过高或无盘口者。只有完整新鲜盘口的 Best Ask 位于用户配置的 `0.1¢–99¢` 最低/最高买价闭区间、存在有效 Best Bid、Bid/Ask 比例达到配置（默认 50%）、tick 和最小下单量有效时，才允许按 FAK 买入。
...
- 买入可部分成交，剩余立即取消。每笔 Fill 按 `min(99¢, tick 向上取整[max(买价+A, 买价×B)])` 创建目标卖价；`A/B` 可在配置页修改，默认仍为 `+1¢ / ×1.5`，已有目标卖单不追溯改写。
- Best Bid 达标后立即按 FAK 卖出；买入完成时也会在同一盘口立即复查。部分卖出继续等待，首次卖出后本轮停止新买。
- 主动止损默认开启，默认线为本轮实际成交加权买入均价的 `0.40×`，倍数可在配置页修改。新鲜可成交 Best Bid 严格低于止损线、由不同盘口版本连续确认约 30 秒后，程序原子取消目标卖单并按 FAK 主动退出；没有深度或仅部分成交时保持退出状态继续重试。止损一旦触发，该 Event 在本次 TEST 内不再重入。
...
- 默认展示总资金、已实现、未实现、持仓价值、单数、当前持仓、默认折叠的交易记录和扫描 Event，配置按需展开；
- 配置包括 Polymarket 首页栏目、二元/三元/多元（4+）、`0.1¢–99¢` 最低/最高买价、完整目标卖价公式及可配置 `A/B` 参数、可配置主动止损开关/倍数、Bid/Ask 比例、最大生命周期进度、总时长自由区间、总模拟资金和每 Event 每轮金额；
- 符合配置的市场全部自动参与，不提供逐 Token 勾选；
- 当前持仓显示 Event、active Token、冻结预算、已用金额、止损线和累计/止损确认/止损退出状态；持仓与扫描 Event 均默认显示前 20 项并可展开；扫描 Event 固定将可交易项排在待定项前面，两组内部使用一个小按钮切换生命周期进度正序/倒序；
