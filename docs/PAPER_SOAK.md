...
最后一行`summary`记录总样本数、告警样本数、本机采样失败数、最大扫描分页/批次/规模/耗时、事件页与订单簿请求峰值、扫描重试/限流/临时错误峰值、最大订阅量、进程运行时长与内存峰值、行情恢复及PAPER活动计数的首末值、窗口最大值和最终停止原因。异常断线与恢复采用进程内累计计数，因此即使发生在两次采样之间也不会被定时采样完全遗漏；窗口最大值则避免服务器重启后进程计数归零掩盖重启前活动。资源峰值用于比较运行窗口和评估服务器规格，取得正式基线前不作为自动失败阈值。
